...
function convertBrandSize(fB,fSz,tB,cat){
  const fChart=BRAND_SIZES[fB]?.[cat],tChart=BRAND_SIZES[tB]?.[cat];
  if(!fChart||!tChart)return null;
  const fGroups=fChart.groups||[{label:null,sizes:fChart.sizes,idx:fChart.idx}];
  const tGroups=tChart.groups||[{label:null,sizes:tChart.sizes,idx:tChart.idx}];
